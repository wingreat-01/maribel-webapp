...
var DATA_TAB     = 'BEGINNING';  // Beginning inventory
...
// BEGINNING tab actual layout (matches your Google Sheet):
//   A=ItemCode, B=Description, C=Width, D=W-UM, E=Length, F=L-UM,
//   G=BeginningQty, H=StockingUnit, I=Remarks  (Columns J:Q are ignored)
var DATA_HEADER     = ['ItemCode', 'Description', 'Width', 'W-UM', 'Length', 'L-UM', 'BeginningQty', 'StockingUnit', 'Remarks'];
...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'read_data') {
      ...
      var lastCol = dsh.getLastColumn();
      // Read enough columns to reach Remarks in col I (index 8) = 9 columns minimum
      var numCols = Math.max(lastCol, 9);
      var rawRows = lastRow < 1 ? [] : dsh.getRange(1, 1, lastRow, numCols).getValues();

      // Remap sheet columns → the 7-column format the frontend expects:
      // frontend: [ItemCode, Qty, Width, W-UM, Length, L-UM, Remarks]
      // sheet:     A(0)=ItemCode, B(1)=Description, C(2)=Width, D(3)=W-UM,
      //            E(4)=Length,   F(5)=L-UM, G(6)=BeginningQty,
      //            H(7)=StockingUnit, I(8)=Remarks
      var remapped = rawRows.map(function(r, idx) {
        if (idx === 0) {
          return ['ItemCode', 'Description', 'Qty', 'Width', 'W-UM', 'Length', 'L-UM', 'Remarks'];
        }
        return [
          r[0] != null ? r[0] : '',   // ItemCode     (col A)
          r[1] != null ? r[1] : '',   // Description  (col B)
          r[6] != null ? r[6] : 0,    // Qty          (col G = BeginningQty)
          r[2] != null ? r[2] : '',   // Width        (col C)
          r[3] != null ? r[3] : '',   // W-UM         (col D)
          r[4] != null ? r[4] : '',   // Length       (col E)
          r[5] != null ? r[5] : '',   // L-UM         (col F)
          r[8] != null ? r[8] : ''    // Remarks      (col I)
        ];
      });

      output = _json({ok: true, rows: remapped});
      return _addCors(output);
    }
...
function _handleWrite(body) {
  ...
  if (action === 'backup_all') {
    ...
    (b.beginning || []).forEach(function(it) {
      // Write to correct columns: A=ItemCode, G=BeginningQty, I=Remarks
      // B=Description, C=Width, D=W-UM, E=Length, F=L-UM, H=StockingUnit
      drows.push([
        String(it.code || ''),      // A: ItemCode
        String(it.desc || ''),      // B: Description
        String(it.width || ''),     // C: Width
        String(it.widthUm || ''),   // D: W-UM
        String(it.length || ''),    // E: Length
        String(it.lengthUm || ''),  // F: L-UM
        Number(it.qty) || 0,        // G: BeginningQty ← THE FIX
        String(it.unit || ''),      // H: StockingUnit
        String(it.remarks || '')    // I: Remarks
      ]);
    });
...
// Insert or update a row in the BEGINNING sheet.
// Sheet layout: A=ItemCode, B=Description, C=Width, D=W-UM, E=Length, F=L-UM,
//               G=BeginningQty, H=StockingUnit, I=Remarks
// Only columns A and C:I are written — column B (Description) and H (StockingUnit)
...
function _upsert(sh, code, qty, width, widthUm, length, lengthUm, remarks) {
  ...
  if (rowIdx === -1) {
    // New row: write all 9 columns (Description and StockingUnit left blank)
    sh.appendRow([String(code), '', w, wu, l, lu, Number(qty) || 0, '', rem]);
  } else {
    // Update: read existing row to preserve Description (B) and StockingUnit (H)
    var existing = sh.getRange(rowIdx, 1, 1, 9).getValues()[0];
    var desc     = existing[1] || '';   // B: keep existing Description
    var stockUm  = existing[7] || '';   // H: keep existing StockingUnit
    sh.getRange(rowIdx, 1, 1, 9).setValues([[
      String(code),         // A: ItemCode
      desc,                 // B: Description (preserved)
      w,                    // C: Width
      wu,                   // D: W-UM
      l,                    // E: Length
      lu,                   // F: L-UM
      Number(qty) || 0,     // G: BeginningQty ← THE FIX
      stockUm,              // H: StockingUnit (preserved)
      rem                   // I: Remarks
    ]]);
  }
}
...
// Add CORS headers — passthrough kept for clarity and future flexibility.
